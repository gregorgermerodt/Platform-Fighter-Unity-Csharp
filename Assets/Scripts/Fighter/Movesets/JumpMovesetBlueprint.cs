...
public class JumpMovesetBlueprint : IMovesetBlueprint
{
    ...
    private static void JUMP_RISING_ACMD(FighterMoveset fm)
    {
        ...
        if (fm.OnFrame(6))
        {
            if (fm.IsFlagTrue("PERFORMING_FULLHOP_FLAG"))
                fm.fighterController.ApproachVerticalVelocity(FULLHOP_DECELERATION, 0.0f);
            else if (fm.IsFlagTrue("PERFORMING_SHORTHOP_FLAG"))
                fm.fighterController.ApproachVerticalVelocity(SHORTHOP_DECELERATION, 0.0f);
